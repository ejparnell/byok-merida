...
export async function collectCaptureEvidence() {
  ...
  const evidence = {
    ...main,
    selectedText: frames.map((frame) => frame.selectedText).find(Boolean) || '',
    visibleText: frames.map((frame) => frame.visibleText).filter(Boolean).join('\n\n').slice(0, 120000),
    semanticHtml: frames.map((frame) => frame.semanticHtml).filter(Boolean).join('\n').slice(0, 120000),
  }

...
- The final app has one local-first real runtime. Credential-free verification uses explicitly injected test fakes behind the same workflow interfaces; there is no selectable demo product mode, fixture workspace, or reset API.
...
- [Remove demo mode from the final app](issues/09-define-demo-mode-acceptance.md) — The final app now has one real Notion and DeepSeek composition; demo settings, fixtures, persistence, reset API, UI, and generated-client surface are removed, while credential-free acceptance remains behind explicitly injected test fakes at the ASGI/OpenAPI seam.

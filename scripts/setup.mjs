...
function readVersion(command, args, label) {
  ...
  if (result.error || result.status !== 0) {
    console.error(`${label} is required for the Merida toolchain.`)
    process.exit(1)
  }
...
function run(command, args) {
  ...
  if (result.error) {
    console.error(`${command} is required for the Merida toolchain.`)
    process.exit(1)
  }
...
if (!uvProbe.error && uvProbe.status === 0) {
  ...
  console.error(
    'uv 0.11.28 or newer is required for a clean setup. Install uv, then rerun npm run setup.',
  )
  process.exit(1)
} else {
  console.log(
